    ...
    Private Function ImportData(ByVal FileName As String, Optional ByRef errMessage As String = "") As Boolean
        ...
        Try
            ...
            For i = 1 To oWb.Worksheets.Count
                If oWb.Worksheets(i).name = "COA" Then
                    checksheet = True
                End If
            Next
            ...
            BackgroundWorker1.ReportProgress(2, "Select Worksheet ""COA""")
            oSheet = oWb.Worksheets("COA")
            ...
            Dim sapaccountfid As Integer = 0
            Dim sapindexfid As Integer = 0
            Dim sapccfid As Integer = 0
            Dim sapindexaccnameid As Integer = 0
            ...
            Dim sapaccnamefid As Integer = 0
            ...
            'delete sapindexf
            DbAdapter1.deletesapindexf()

            For i = 2 To iRows
                If IsNothing(oSheet.Cells(i, 2).value) Then
                    Exit For
                End If
                ...
                sapaccountfid = DbAdapter1.getSapAccountFid(oSheet.Cells(i, 3).value.ToString)
                ...
                sapccfid = DbAdapter1.getsapccfid(oSheet.Cells(i, 11).value.ToString)

                '2. Check table sapindex if not avail then create
                sapindexfid = DbAdapter1.getSapIndexfid(sapaccountfid, sapccfid, oSheet.Cells(i, 2).value.ToString)

                '8. Check table sapaccname if not avail then create

                sapaccnamefid = DbAdapter1.getSapAccNamefId(escString(oSheet.Cells(i, 6).value.ToString))

                'Create record
                'sapindexaccnameid = DbAdapter1.getSapindexaccnameId(oSheet.Cells(i, 6).value.ToString)
                stringbuilder1.Append(sapindexfid & vbTab)
                stringbuilder1.Append(sapaccnamefid & vbCrLf)
            ...
            Dim sqlstr = "delete from sapindexaccnamef;select setval('sapindexaccnamef_sapindexaccnamefid_seq',1,false);copy sapindexaccnamef(sapindexfid,sapaccnamefid) from stdin;"
            BackgroundWorker1.ReportProgress(2, "Copy To Db (sapindexaccname)")
