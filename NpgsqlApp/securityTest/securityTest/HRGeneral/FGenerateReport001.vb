...
Public Class FGenerateReport001
    ...
    Private Function GenerateExcel(ByRef FileName As String, ByRef errorMsg As String) As Boolean
        ...
        Try
            ...
            If MyRegionId = 1 Then
                oWb = oXl.Workbooks.Open(Application.StartupPath & "\templates\ExcelTxTemplate.xltx")
            Else
    ...
    Public Sub QueryData(ByRef owb As Excel.Workbook, ByVal isheet As Integer)
        ...
        Next
        Dim firstdate As String = "'" & DateTimePicker1.Value.Year & "-" & DateTimePicker2.Value.Month & "-1'"
        Dim lastdate As String = "'" & DateTimePicker1.Value.Year & "-" & DateTimePicker3.Value.Month & "-" & getlastdate(DateTimePicker1.Value.Year, DateTimePicker3.Value.Month) & "'"
        ...
        If MyRegionId <> 0 Then
            ...
            End If
            If MyRegionId = 1 Then
                'sqlstr = "select * from fgetbudgettxlc(" & VersionId & "," & firstdate & "::date," & lastdate & "::date," & MyRegionId & ") as b(sapaccname character varying,categoryfinance character varying,expensesnature character varying, sapaccount character varying, sapaccid character varying, sapcc character varying, dept character varying, category character varying, joindate date,enddate date, personname character varying, othername character varying, title character varying, expat boolean,mydate date,amount numeric,headcount numeric,validheadcount numeric,regionname character varying,crcytype character varying,version character varying);"
                stringbuilder1.Append("select * from fgetbudgettxlctitle(" & VersionId & "," & firstdate & "::date," & lastdate & "::date," & MyRegionId & ") as b(sapaccname character varying,categoryfinance character varying,expensesnature character varying, sapaccount character varying, sapaccid character varying, sapcc character varying, dept character varying, category character varying, joindate date,enddate date, effectivedatestart date,effectivedateend date,bonusfactor integer,personname character varying, othername character varying, title character varying, expat boolean,mydate date,amount numeric,headcount numeric,validheadcount numeric,regionname character varying,crcytype character varying,version character varying)")
                stringbuilder1.Append(" union all ")
                stringbuilder1.Append("select * from fgetbudgettxusdtitle(" & VersionId & "," & firstdate & "::date," & lastdate & "::date," & MyRegionId & ") as b(sapaccname character varying,categoryfinance character varying,expensesnature character varying, sapaccount character varying, sapaccid character varying, sapcc character varying, dept character varying, category character varying, joindate date,enddate date,effectivedatestart date,effectivedateend date,bonusfactor integer, personname character varying, othername character varying, title character varying, expat boolean,mydate date,amount numeric,headcount numeric,validheadcount numeric,regionname character varying,crcytype character varying,version character varying);")
            Else
            ...
            If VersionId = 0 Then
                ...
                For Each rec In Dataset1.Tables(0).Rows
                    Try
                        ...
                        'sqlstr = "(select * from fgetbudgettxlc(" & VersionId & "," & firstdate & "::date," & lastdate & "::date," & rec.item("regionid") & ") as b(sapaccname character varying,categoryfinance character varying,expensesnature character varying, sapaccount character varying, sapaccid character varying, sapcc character varying, dept character varying, category character varying, joindate date,enddate date, personname character varying, othername character varying, title character varying, expat boolean,mydate date,amount numeric,headcount numeric,validheadcount numeric,regionname character varying,crcytype character varying,version character varying))"
    ...
    Public Sub QueryDataOld(ByRef owb As Excel.Workbook, ByVal isheet As Integer)
        Dim sqlstr As String = String.Empty
        Dim stringbuilder1 As New System.Text.StringBuilder
    ...
    Private Sub GeneratePivotTable(ByVal oWb As Excel.Workbook, ByVal iSheet As Integer, ByRef errMsg As String)
        ...
        Try
            ...
            If MyRegionId = 1 Then
                osheet.PivotTables("PivotTable1").PivotFields("effectivedatestart").orientation = Excel.XlPivotFieldOrientation.xlRowField
                osheet.PivotTables("PivotTable1").PivotFields("effectivedateend").orientation = Excel.XlPivotFieldOrientation.xlRowField                
            End If
            ...
            osheet.PivotTables("PivotTable1").PivotFields("Months").numberformat = "MMM"
            ...
            If MyRegionId = 1 Then
                osheet.PivotTables("PivotTable1").PivotFields("effectivedatestart").Subtotals = {False, False, False, False, False, False, False, False, False, False, False, False}
                osheet.PivotTables("PivotTable1").PivotFields("effectivedateend").Subtotals = {False, False, False, False, False, False, False, False, False, False, False, False}
            End If
            ...
            'hide usd
            For Each mypv As Excel.PivotItem In osheet.PivotTables("PivotTable1").PivotFields("crcytype").PivotItems
                If mypv.Value = "USD" Then
                    mypv.Visible = False
                End If
            Next
            ...
            If MyRegionId = 1 Then
                ...
                osheet = oWb.Worksheets(iSheet)
                osheet.Name = "Summary"
                oWb.Worksheets("PivotTable").PivotTables("PivotTable1").PivotCache.CreatePivotTable(osheet.Name & "!R7C1", "PivotTable1", Excel.XlPivotTableVersionList.xlPivotTableVersionCurrent)
                ...
                osheet.PivotTables("PivotTable1").PivotFields("crcytype").Orientation = Excel.XlPivotFieldOrientation.xlRowField
                osheet.PivotTables("PivotTable1").PivotFields("sapaccname").Orientation = Excel.XlPivotFieldOrientation.xlRowField
                osheet.PivotTables("PivotTable1").PivotFields("categoryfinance").Orientation = Excel.XlPivotFieldOrientation.xlRowField
                osheet.PivotTables("PivotTable1").PivotFields("expensesnature").Orientation = Excel.XlPivotFieldOrientation.xlRowField
                osheet.PivotTables("PivotTable1").PivotFields("sapaccount").Orientation = Excel.XlPivotFieldOrientation.xlRowField
                osheet.PivotTables("PivotTable1").PivotFields("sapcc").Orientation = Excel.XlPivotFieldOrientation.xlRowField
                osheet.PivotTables("PivotTable1").PivotFields("sapaccid").Orientation = Excel.XlPivotFieldOrientation.xlRowField
                osheet.PivotTables("PivotTable1").PivotFields("dept").Orientation = Excel.XlPivotFieldOrientation.xlRowField
                osheet.PivotTables("PivotTable1").PivotFields("regionname").Orientation = Excel.XlPivotFieldOrientation.xlRowField
                osheet.PivotTables("PivotTable1").PivotFields("mydate").Orientation = Excel.XlPivotFieldOrientation.xlRowField
                osheet.PivotTables("PivotTable1").PivotFields("version").Orientation = Excel.XlPivotFieldOrientation.xlRowField

                osheet.PivotTables("PivotTable1").PivotFields("crcytype").subtotals = {False, False, False, False, False, False, False, False, False, False, False, False}
                osheet.PivotTables("PivotTable1").PivotFields("sapaccname").subtotals = {False, False, False, False, False, False, False, False, False, False, False, False}
                osheet.PivotTables("PivotTable1").PivotFields("categoryfinance").subtotals = {False, False, False, False, False, False, False, False, False, False, False, False}
                osheet.PivotTables("PivotTable1").PivotFields("expensesnature").subtotals = {False, False, False, False, False, False, False, False, False, False, False, False}
                ...
                osheet.PivotTables("PivotTable1").PivotFields("sapaccid").subtotals = {False, False, False, False, False, False, False, False, False, False, False, False}
                osheet.PivotTables("PivotTable1").PivotFields("dept").subtotals = {False, False, False, False, False, False, False, False, False, False, False, False}
                osheet.PivotTables("PivotTable1").PivotFields("regionname").subtotals = {False, False, False, False, False, False, False, False, False, False, False, False}
                osheet.PivotTables("PivotTable1").PivotFields("mydate").subtotals = {False, False, False, False, False, False, False, False, False, False, False, False}
                osheet.PivotTables("PivotTable1").PivotFields("version").subtotals = {False, False, False, False, False, False, False, False, False, False, False, False}

                osheet.PivotTables("PivotTable1").AddDataField(osheet.PivotTables("PivotTable1").PivotFields("amount"), "BUD", Excel.XlConsolidationFunction.xlSum)
                osheet.PivotTables("PivotTable1").AddDataField(osheet.PivotTables("PivotTable1").PivotFields("validheadcount"), "HC", Excel.XlConsolidationFunction.xlSum)

                osheet.PivotTables("PivotTable1").RepeatAllLabels(Excel.XlPivotFieldRepeatLabels.xlRepeatLabels)
            End If
            osheet.Cells.EntireColumn.AutoFit()
        Catch ex As Exception
            errMsg = ex.Message
    ...
    'Private Function getlastdate(ByVal p1 As Integer, ByVal p2 As Integer) As String
    '    If p2 = 12 Then
    '        p1 = p1 + 1
    '        p2 = 1
    '    Else
